...
func TestVerifyManifest_RejectsDuplicateAndSymlinkEntries(t *testing.T) {
	...
	if _, verifyErr := VerifyManifest(manifestPath, tmpDir); verifyErr == nil {
		t.Fatal("VerifyManifest should reject duplicate entries")
	}

	linkPath := filepath.Join(tmpDir, "artifact-link.bin")
	if symlinkErr := os.Symlink(filePath, linkPath); symlinkErr != nil {
	...
	if writeErr := os.WriteFile(outsideFile, []byte("content"), 0644); writeErr != nil {
		t.Fatal(writeErr)
	}
	if symlinkErr := os.Symlink(outsideDir, filepath.Join(tmpDir, "linked-dir")); symlinkErr != nil {
